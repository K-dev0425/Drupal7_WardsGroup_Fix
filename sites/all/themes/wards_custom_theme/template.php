 ...
 */
function wards_custom_preprocess_maintenance_page(&$vars) {
  // While markup for normal pages is split into page.tpl.php and html.tpl.php,
  // the markup for the maintenance page is all in the single
  // maintenance-page.tpl.php template. So, to have what's done in
  // adminimal_preprocess_html() also happen on the maintenance page, it has to be
  // called here.
  wards_custom_preprocess_html($vars);
}

/**
 * Override or insert variables into the html template.
 */
function wards_custom_preprocess_html(&$vars) {

  // Get adminimal folder path.
  $adminimal_path = drupal_get_path('theme', 'wards_custom');
 ...
 */
function wards_custom_preprocess_page(&$vars) {
 ...
 */
function wards_custom_node_add_list($variables) {
 ...
 */
function wards_custom_adminimal_block_content($variables) {
  $content = $variables['content'];
  $output = '';
  if (!empty($content)) {
 ...
 */
function wards_custom_tablesort_indicator($variables) {
 ...
 */
function wards_custom_css_alter(&$css) {
 ...
 */
function wards_custom_js_alter(&$javascript) {
 ...
 */
function wards_custom_admin_block($variables) {
 ...
 * Adding classes to the administration blocks see issue #1869690.
 */
function wards_custom_admin_block_content($variables) {
  $content = $variables['content'];
  $output = '';
 ...
 */
function wards_custom_table($variables) {
 ...
 */
function wards_custom_file_widget_multiple($variables) {
    ...
    foreach ($widgets as $key => &$widget) {
        ...
        $row[] = $weight;
        $row[] = format_date($widget['#file'] -> timestamp);
        $row[] = $operations;
        $rows[] = array(
            'data' => $row,
