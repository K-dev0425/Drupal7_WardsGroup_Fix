<?php
 // manifest print tpl
  ...
  $dis_time = date('H:i:s', $manifest->created);
  ...
  $type_tid=$manifest->field_manifest_type['und'][0]['tid'];
  ...
  $trailer=$manifest->field_trailer['und'][0]['value'];
?>
...
<table style="width:100%;margin-bottom:10px;font-family: 'Open Sans';font-size: 12px;">
  ...
  <tr>
    ...
    <td style="padding: 10px 20px;text-align: right;">Dispatch time : <b><?php echo $dis_time; ?></b></td>
  </tr>
</table>
<table style="width:100%;">
    <thead>
      <tr>
        ...
        <th style="background:  #fff;font-family: Open Sans;font-size: 12px;font-weight: 300;border-top:1px solid #000;border-left:1px solid #000; border-bottom:1px solid #000;padding: 10px;color: #000;">Origin Branch</th>
        <th style="background:  #fff;font-family: Open Sans;font-size: 12px;font-weight: 300;border-top:1px solid #000;border-left:1px solid #000; border-bottom:1px solid #000;padding: 10px;color: #000;">Destination Branch</th>
        ...
        <th style="background:  #fff;font-family: Open Sans;font-size: 12px;font-weight: 300;border-top:1px solid #000;border-left:1px solid #000; border-bottom:1px solid #000;padding: 10px;color: #000;">Delivery Notes</th>
  ...
  <?php
    foreach($manifest->field_avaiable_jobs[und] as $jid) {
      //echo"<pre>"; print_r($jid['target_id']); die();
        $job = node_load($jid['target_id']);

        if($job->field_receiver_manual_entry['und'][0]['value'] == 1){
            $des_id=$job->field_receiver_third_party['und'][0]['value'];
            $des_load = field_collection_item_load($des_id);
            $receiver_name = $des_load->field_receiver_manual_name['und'][0]['value'];
        }
        else {
        ...
        $load_sender_br=taxonomy_term_load($sender_branch_id);
        $sender_branch=$load_sender_br->name;

        $delivery_notes_id = $job->field_delivery_notes['und'][0]['target_id'];
        $load_notes_br=taxonomy_term_load($delivery_notes_id);
        $delivery_notes=$load_notes_br->title;

        $receiver_branch_id = $job->field_receiver_branch['und'][0]['target_id'];
        $load_receiver_br=taxonomy_term_load($receiver_branch_id);
        $receiver_branch=$load_receiver_br->name;

        $assign_driver_id=$job->field_assigned_driver['und'][0]['target_'];
        ...
        ?>
            <tbody><tr>
              ...
              <td style="padding: 10px;border-left:1px solid #000;  border-bottom:1px solid #000;"><?php echo $sender_branch;?></td>
              ...
              <td style="padding: 10px;border-left:1px solid #000; border-bottom:1px solid #000;"><?php echo $delivery_notes ;?></td>
        ...
        <?php } }?>
            ...
            <tr style="background-color: #fff;">
                ...
                <td style="padding: 10px;"></td><td></td><td></td><td></td>
